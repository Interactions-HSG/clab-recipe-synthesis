...
for obj_id in range(NOBJ):
    problem.set_initial_value(block_size(domain_objects[f"{TERMELEM}{obj_id}"]), 1 + obj_id)
...
if 3 == NOBJ:
    ...
    problem.add_goal(
        LE(
            ...
            Plus(
                Plus(
                    piece_dim(domain_objects[f"{TERMGOAL}0"]),
                    piece_dim(domain_objects[f"{TERMGOAL}1"])
                ),
                piece_dim(domain_objects[f"{TERMGOAL}2"]))
        )
    )
elif 2 == NOBJ:
    problem.add_goal(
