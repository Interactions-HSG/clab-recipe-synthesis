...
on = Fluent("on", BoolType(), sh_a=Block, sh_b=Block)
clear = Fluent("clear", BoolType(), sh_clear=Block)
# Left-handed CD - gain orientation in frame without braking your fingers
size_y_of = Fluent("size_y_of", IntType(), sh_size=Block)
...
attach_shape = InstantaneousAction("attach", blk=Block, sh_dst=Block)
...
detach_shape = InstantaneousAction("detach", sh_from=Block, blk=Block)
...
# problem.add_goal(Equals(size_y_of(problem.object("tower")),3))
problem.add_goal(Equals(size_y_of(tower),3))
